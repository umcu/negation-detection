                ...
                for example in f.read().split("\n\n"):
                    ...
                    for i in range(len(lines)):
                        ...
                        if not line or i == len(lines) - 1:
                            if len(sentence) > 0:
                                if (
                                    len(tokenizer.tokenize(" ".join(sentence)))
                                    <= args.block_size - 2
                                ):
...
class TextDatasetFromDataFrame(Dataset):
    ...
    def __init__(self, df, tokenizer, args):
        ...
        for id in df.Id.unique():
            ...
            if args.task == "negation":
                labels = [
                    row[2] + "-" + row[3] if row[2] != "O" else "O" for row in lines
                ]
            elif args.task == "experiencer":
                labels = [
                    row[2] + "-" + row[4] if row[2] != "O" else "O" for row in lines
                ]
            elif args.task == "temporality":
                labels = [
                    row[2] + "-" + row[5] if row[2] != "O" else "O" for row in lines
                ]
            _ids = [
                row[0] + "_" + row[6] + "_" + row[7] if row[2] != "O" else "O"
                for row in lines
            ]

            if len(sentence) > 0:
                if (
                    len(tokenizer.tokenize(" ".join(sentence))) <= args.block_size - 2
                ):
                    self.data.append(" ".join(sentence))
                    self.y.append(" ".join(labels))
                    ...
                    for i in range(len(sentence)):
                        ...
                        if length + len(tokens) <= (args.block_size - 2) and i != len(
                            sentence
                        ):
                            sub_sentence.append(sentence[i])
                            sub_labels.append(labels[i])
...
def create_train_batch(
    sentences, labels, tag2id, device, tokenizer, model_type, max_len, word_dropout=0.0
):
    """
    ...
    tok = [
        [tokenizer.bos_token] + tokenizer.tokenize(sen) + [tokenizer.eos_token]
    ...
    # What the flying fuckaroo is going on here? : ignore_index is the id that is used to flag the padding for the optimizer
    ...
    for i in range(len(input_ids)):
        assert len(input_ids[i]) == len(
            label_ids[i]
        ), f"Length of inputs and labels differs. Inputs:{input_ids[i]}, Labels: {label_ids[i]}"
...
def create_eval_batch(
    ...
    for i in range(len(input_ids)):
        assert len(input_ids[i]) == len(
            label_ids[i]
        ), f"Length of inputs and labels differs. \
    ...
    try:
        ...
        print(
            f"input len: {len(input_ids)}, label len: {len(label_ids)}, true ids len: {len(true_ids)}"
        )
        raise
...
def eval_model(model, eval_dataset, tag2id, device, tokenizer, args, return_pred=False):
    ...
    with torch.no_grad():
        for sents, labs, _ids in dl:
            ...
            x_in, y, seq_mask, batch_ids = create_eval_batch(
                sents,
                labs,
                _ids,
                tag2id,
            ...
            if preds is None:
                ...
                out_label_ids = np.append(
                    out_label_ids, y.detach().cpu().numpy(), axis=0
                )
                ids.extend(batch_ids)
    ...
    try:
        ...
        print(
            f"preds len: {len(preds)}, out_label_ids len: {len(out_label_ids)}, ids len: {len(ids)}"
        )
    ...
    if args.bootstrap:
        ...
        print(
            ...
            % (
                np.mean(precisions),
                np.percentile(precisions, 2.5),
                np.percentile(precisions, 97.5),
            )
        )
        print(
            "Recall: %.2f (%.2f - %.2f)"
...
def train_model(
    ...
    for epoch_num in range(1, args.num_epochs + 1):
        ...
        for sents, labs, _ in tqdm(
            dl, desc="Epoch %i" % epoch_num, position=0, leave=True
        ):
            # set model in training mode and create batch
            model.train()
            x_in, y, seq_mask = create_train_batch(
                sents, labs, tag2id, device, tokenizer, args.model_type, args.block_size
            )
            ...
            if args.do_eval and step % args.eval_steps == 0:
                print(
                    "Epoch %d, step %d: training loss = %.2f"
                    % (epoch_num, step, tr_loss / epoch_steps)
                )

                val_f, val_p, val_r = eval_model(
                    model=model,
...
def main():
    ...
    parser.add_argument("--centered_block", type=bool, default=False)
    ...
    model = AutoModelForTokenClassification.from_pretrained(
        args.model_path, num_labels=len(tag2id)
    )
