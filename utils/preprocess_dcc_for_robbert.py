...
def get_dataset(textfiles, labelfiles, nrs):
    ...
    for text, labels, nr in zip(textfiles, labelfiles, nrs):
        ...
        for i in range(len(text)):
            ...
            if c in [' ','\r','\n','.',',','!','?',':',';','-','/','\"','\'','(',')','#','>','<','+','=','|', '[',']']:
                #if empty continue
                if max(len(set(bio_tag)), len(set(negation)), len(set(experiencer)), \
                        len(set(temporality)), len(set(beginning)), len(set(ending))) == 0:
                    continue
                
                # if end of word: append to dataset
                elif max(len(set(bio_tag)), len(set(negation)), len(set(experiencer)), 
                        len(set(temporality)), len(set(beginning)), len(set(ending)))== 1:
                    
                    # append data
                    data.append([nr, word, bio_tag[0], negation[0], experiencer[0], temporality[0], beginning[0], ending[0]])
                    word, bio_tag, negation, experiencer, temporality, beginning, ending = '', [], [], [], [], [], []
                ...
                if c in ['.',',','!','?',':',';','-','/','\'','\"','(',')','#','>','<','+','=','|', '[',']']:
                    if not first_word and bio[i] == 1:
                        bio[i] = 2
                    data.append([nr, c, bio[i], neg[i], exp[i], tem[i], begin[i], end[i]])
        ...
        for line in text:
            ...
            for i in range(2,8):
                if line[i] == 0:
                    ...
                    if i == 2: example += "B"
                    if i == 3: example += "Negated"
                    if i == 4: example += "Patient"
                    if i == 5: example += "Recent"
                elif line[i] == 2:
                    if i == 2: example += "I"
                    if i == 3: example += "NotNegated"
                    if i == 4: example += "Other"
                    if i == 5: example += "Historical"
        ...
        for line in text:
            ...
            for i in range(2,8):
                if line[i] == 0:
                    ...
                    if i == 2: line_dict['BIO']= "B"
                    if i == 3: line_dict['Negation'] = "Negated"
                    if i == 4: line_dict['Experiencer'] = "Patient"
                    if i == 5: line_dict['Temporality'] = "Recent"
                elif line[i] == 2:
                    if i == 2: line_dict['BIO']= "I"
                    if i == 3: line_dict['Negation']= "NotNegated"
                    if i == 4: line_dict['Experiencer']= "Other"
                    if i == 5: line_dict['Temporality']= "Historical"
                elif line[i] == 3 and i == 5:
...
def main():
    ...
    exclusion_list_from_file = [d['name'] for d in json.load(open('../DCC_files_to_exclude.json'))['annotation_errors']]
    exclusion_list_from_file += [d['name'] for d in json.load(open('../DCC_files_to_exclude.json'))['corrupted']]
