...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_folder', dest='output_folder', type=str)
    parser.add_argument('--source', dest='source', help='Cleaning settings', type=str)
    ...
    for _cat in df.article_category.unique():
        ...
        dfc.fillna('')
        with open(os.path.join(args.output_folder, 'category', _cat, ".txt"), 'w', encoding='utf-8') as fw:
            for txt in dfc[['body_txt', 'samenvatting_txt']].sum(axis=1):
                fw.write(txt+"\n\r")
    ...
    for _type in df.article_type.unique():
        ...
        dfc.fillna('')
        with open(os.path.join(args.output_folder, 'article_type', _type, ".txt", encoding='utf-8'), 'w') as fw:
            for txt in dfc[['body_txt', 'samenvatting_txt']].sum(axis=1):
                fw.write(txt+"\n\r")