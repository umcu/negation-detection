...
def create_batch(sentences, labels, tag2id, device, tokenizer, model_type, max_len, word_dropout=0.):
    ...
    for i in range(len(input_ids)):
        assert len(input_ids[i]) == len(label_ids[i]), "Length of inputs and labels differs"
     ...
     for epoch_num in range(1, args.num_epochs + 1):
         ...
         for sents, labs in tqdm(dl, desc="Epoch %i" % epoch_num, position=0, leave=True):
             ...
             if args.do_eval and step % args.eval_steps == 0:
                 ...
                 val_f = eval_model(model = model, eval_dataset = eval_dataset, tag2id = tag2id, device = device, tokenizer = tokenizer, args = args)
                 if val_f > best_val_f:
                     best_val_f = val_f
...
def main():
    ...
    if args.do_train:
        train_model(model = model, tokenizer = tokenizer, train_dataset = train_dataset, eval_dataset = eval_dataset, tag2id = tag2id, device = device, args = args)
    
    if args.do_eval:
        eval_model(model = model, tokenizer = tokenizer, eval_dataset = eval_dataset, tag2id = tag2id, device = device, args = args)
